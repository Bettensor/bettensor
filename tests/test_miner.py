...
def test_forward(mock_bettensor_miner):
    ...
    with patch.object(mock_bettensor_miner.games_handler, "process_games") as mock_process_games, \
         patch.object(mock_bettensor_miner.predictions_handler, "process_predictions") as mock_process_predictions:
...
def test_get_active_games(mock_games_handler):
    ...
    mock_cursor.fetchall.return_value = [("game1", "TeamA", 1.5, "TeamB", 2.5, "Sport", "League", "ext1", "2023-01-01", "2023-01-01", "2023-01-02", 1, None, 3.0, True)]
    mock_games_handler.db_manager.get_cursor.return_value.__enter__.return_value = mock_cursor
...
def test_process_predictions(mock_predictions_handler):
    updated_games = {"game1": TeamGame(id="game1", teamA="TeamA", teamB="TeamB", externalId="ext1")}
    new_games = {"game2": TeamGame(id="game2", teamA="TeamC", teamB="TeamD", externalId="ext2")}
    
    with patch.object(mock_predictions_handler, "_update_prediction_outcome") as mock_update, \
         patch.object(mock_predictions_handler, "_get_or_create_prediction") as mock_get_or_create:
        mock_get_or_create.return_value = TeamGamePrediction(predictionID="pred1", teamGameID="game2")
...
def test_load_state(mock_state_manager):
    with patch.object(mock_state_manager, "_load_state_from_file") as mock_file_load, \
         patch.object(mock_state_manager, "_load_state_from_db") as mock_db_load, \
         patch.object(mock_state_manager, "_get_initial_state") as mock_initial_state:
...
def test_init_miner_row(mock_stats_handler):
    ...
    mock_stats_handler.db_manager.get_cursor.return_value.__enter__.return_value = mock_cursor
