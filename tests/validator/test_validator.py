...
class TestBettensorValidator(unittest.TestCase):
    ...
    def test_get_uids_to_query(self):
        ...
        (
            ...
            blacklisted_uids,
            uids_not_to_query,
        ) = self.validator.get_uids_to_query(all_axons)
    ...
    def test_process_prediction(self, mock_connect):
        ...
        expected_calls = [
            ...
            call(
                "SELECT sport, league, event_start_date, team_a, team_b, team_a_odds, team_b_odds, tie_odds, outcome FROM game_data WHERE id = ?",
                ("game_1",),
            ),
            call(
                "SELECT COUNT(*) FROM predictions WHERE prediction_id = ?",
    ...
    def test_determine_winner(self, mock_process_game_result, mock_get_sport_from_db):
        ...
        mock_process_game_result.assert_called_once_with(
            "soccer", mock_game_data["response"][0], "ext_1", "Team A", "Team B"
        )
    ...
    def test_process_game_result(self):
        with patch.object(
            BettensorValidator, "update_game_outcome"
        ) as mock_update_game_outcome:
            sport = "soccer"
            game_response = {"goals": {"home": 2, "away": 1}}
            external_id = "ext_1"
            team_a = "Team A"
            team_b = "Team B"

            self.validator.process_game_result(
                sport, game_response, external_id, team_a, team_b
            )
    ...
    def test_update_recent_games(self, mock_connect):
        ...
        with patch.object(
            BettensorValidator, "determine_winner"
        ) as mock_determine_winner:
    ...
    def test_set_weights(self):
        ...
        self.validator.weight_setter.set_weights.assert_called_once_with(
