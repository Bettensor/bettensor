...
class TestNFLPredictions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.predictions_handler = PredictionsHandler(self.db_manager, self.state_manager, 'test_hotkey')
        self.predictions_handler.stats_handler = self.miner_stats_handler

        preprocessor_path = os.path.join(os.path.dirname(__file__), '..', 'bettensor', 'miner', 'models', 'preprocessor.joblib')
        calibrated_model_path = os.path.join(os.path.dirname(__file__), '..', 'bettensor', 'miner', 'models', 'calibrated_sklearn_model.joblib')
    ...
    def test_process_model_predictions_nfl(self, mock_joblib_load, mock_get_HFmodel):
        ...
        preprocessor_path = os.path.join(os.path.dirname(__file__), '..', 'bettensor', 'miner', 'models', 'preprocessor.joblib')
        team_averages_path = os.path.join(os.path.dirname(__file__), '..', 'bettensor', 'miner', 'models', 'team_historical_stats.csv')
        calibrated_model_path = os.path.join(os.path.dirname(__file__), '..', 'bettensor', 'miner', 'models', 'calibrated_sklearn_model.joblib')
        ...
        mock_miner_stats_handler.update_stats_from_predictions.side_effect = lambda x: None

        predictions_handler = PredictionsHandler(mock_db_manager, self.state_manager, 'test_hotkey')
        ...
        model_path = os.path.join(os.path.dirname(__file__), '..', 'bettensor', 'miner', 'models', 'nfl_wager_model.pt')
        ...
        if os.path.isdir(model_path):
            ...
            raise FileNotFoundError(f"The model directory does not exist at {model_path}")
        ...
        games = {
            'game1': TeamGame(
                id='game1', teamA='New England Patriots', teamB='Buffalo Bills',
                teamAodds=3.8, teamBodds=3.1, tieOdds=1.0,
                sport='nfl', league='NFL', externalId='ext1',
                createDate=current_time, lastUpdateDate=current_time,
                eventStartDate=current_time, active=True, outcome='Unfinished',
                canTie=False,
                schedule_week=1
            ),
            'game2': TeamGame(
                id='game2', teamA='Green Bay Packers', teamB='Chicago Bears',
                teamAodds=1.9, teamBodds=2.0, tieOdds=3.0,
                sport='nfl', league='NFL', externalId='ext2',
                createDate=current_time, lastUpdateDate=current_time,
                eventStartDate=current_time, active=True, outcome='Unfinished',
                canTie=False,
                schedule_week=1
            ...
            'game3': TeamGame(
                id='game3', teamA='Dallas Cowboys', teamB='Philadelphia Eagles',
                teamAodds=2.2, teamBodds=1.7, tieOdds=3.5,
                sport='nfl', league='NFL', externalId='ext3',
                createDate=current_time, lastUpdateDate=current_time,
                eventStartDate=current_time, active=True, outcome='Unfinished',
                canTie=False,
                schedule_week=1 
            ...
            'game4': TeamGame(
                id='game4', teamA='Kansas City Chiefs', teamB='Las Vegas Raiders',
                teamAodds=2.5, teamBodds=1.3, tieOdds=4.0,
                sport='nfl', league='NFL', externalId='ext4',
                createDate=current_time, lastUpdateDate=current_time,
                eventStartDate=current_time, active=True, outcome='Unfinished',
                canTie=False,
                schedule_week=1 
            ...
            'game5': TeamGame(
                id='game5', teamA='San Francisco 49ers', teamB='Seattle Seahawks',
                teamAodds=1.6, teamBodds=2.5, tieOdds=3.8,
                sport='nfl', league='NFL', externalId='ext5',
                createDate=current_time, lastUpdateDate=current_time,
                eventStartDate=current_time, active=True, outcome='Unfinished',
                canTie=False,
                schedule_week=1 
        ...
        for game_id, prediction in predictions.items():
            ...
            self.assertIn(prediction.predictedOutcome, [prediction.teamA, prediction.teamB])
    ...
    def test_get_best_match_nfl(self):
        ...
        encoded_teams = {'New England Patriots', 'Buffalo Bills', 'Green Bay Packers', 'Chicago Bears'}
        
        match = self.predictions_handler.get_best_match('New England Patriots', encoded_teams, 'nfl')
        self.assertEqual(match, 'New England Patriots')

        match = self.predictions_handler.get_best_match('New England', encoded_teams, 'nfl')
        self.assertEqual(match, 'New England Patriots')

        match = self.predictions_handler.get_best_match('Los Angeles Lakers', encoded_teams, 'nfl')
