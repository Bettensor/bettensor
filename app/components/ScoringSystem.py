def scoring_run(self, current_date):
    ...
    if new_day != self.current_day or new_hour != self.current_hour:
        self.logger.info(f"Moving from day {self.current_day}, hour {self.current_hour} to day {new_day}, hour {new_hour}")
...
def manage_tiers(self):
    ...
    try:
        ...
        current_tiers = self._get_tensor_for_day(self.tiers, self.current_day, self.current_hour)
        ...
        self._set_tensor_for_day(self.tiers, self.current_day, self.current_hour, new_tiers)

        # Propagate the new tier information to the next day
        next_day = (self.current_day + 1) % self.max_days
        self.tiers[:, next_day, :] = new_tiers.unsqueeze(1).expand(-1, self.scores_per_day)
...
def _handle_promotions_and_fill_slots(self, tiers, composite_scores):
    ...
    for tier_idx in range(len(self.tier_configs)):
        ...
        tier_scores = composite_scores[tier_mask, self.current_day, self.current_hour, tier_idx]
        if tier_scores.numel() > 0:
            sorted_scores, sorted_indices = tier_scores.sort(descending=True)
            top_miners = sorted_indices[:int(self.tier_configs[tier_idx]['capacity'] * 0.1)]
            # Promote top miners to the next tier
            promote_mask = sorted_indices[:int(self.tier_configs[tier_idx]['capacity'] * 0.1)]
...
def _increment_time(self, new_day, new_hour):
    if new_day != self.current_day:
        ...
        self.tiers[:, target_day, :] = self.tiers[:, source_day, self.current_hour].unsqueeze(1).expand(-1, self.scores_per_day)

    self.current_day = new_day % self.max_days  # Ensure current_day wraps around
    self.current_hour = new_hour % self.scores_per_day  # Ensure current_hour wraps around