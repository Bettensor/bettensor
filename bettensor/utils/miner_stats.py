...
class MinerStatsHandler:
    '''
    This class is used to store miner stats and perform calculations on the data. It is instantiated for each validator.

    '''
    def __init__(self):
        self.db_path = "validator.db"
    ...
    def init_miner_row(self,miner_hotkey:str,miner_coldkey:str,miner_uid:int) -> bool:
        '''
        This method is called when the validator discovers a new miner on the network, or on validator startup for each hotkey.
        ...
        ''', (miner_hotkey, miner_coldkey, miner_uid, miner_rank, 
              miner_cash, miner_current_incentive, miner_last_prediction_date, 
              miner_lifetime_earnings, miner_lifetime_wager, miner_lifetime_predictions, 
              miner_lifetime_wins, miner_lifetime_losses, miner_win_loss_ratio, miner_status))
        self.conn.commit()
        self.conn.close()
        return True
    ...
    def update_miner_row(self,conn,miner:MinerStats) -> bool:
        ...
        miner_status = ?
        ''', (miner_uid, miner_rank, 
              miner_cash, miner_current_incentive, 
              miner_last_prediction_date, miner_lifetime_earnings, 
              miner_lifetime_wager, miner_lifetime_predictions, 
              miner_lifetime_wins, miner_lifetime_losses, 
              miner_win_loss_ratio, miner_status))
        conn.commit()
        conn.close()
        return True
