...
class HealthCheck:
    def __init__(self, db_params, axon_port):
        ...
        bt.logging.info(
            f"Initialized HealthCheck with db_params: {self.db_params}, axon_port: {self.axon_port}"
        )
    ...
    def check_and_open_port(self, port):
        if not self.run_command(["netstat", "-tuln"], f"Failed to check port {port}"):
            self.run_command(
                ["sudo", "ufw", "allow", str(port)], f"Failed to open port {port}"
            )
    ...
    def check_and_setup_redis(self):
        ...
        if not self.run_command(["redis-cli", "ping"], "Redis is not running"):
            ...
            if not self.run_command(
                ["redis-server", "--version"], "Redis is not installed"
            ):
                self.run_command(
            ...
            self.run_command(
                ["sudo", "systemctl", "start", "redis-server"], "Failed to start Redis"
            )
            self.run_command(
            ...
            if os.path.exists(redis_conf):
                ...
                if "bind 0.0.0.0" not in config:
                    bt.logging.info(
                        "Modifying Redis configuration to allow connections from anywhere"
                    )
    ...
    def check_and_setup_postgres(self):
        ...
        self.run_command(
            ["sudo", "systemctl", "start", "postgresql"], "Failed to start PostgreSQL"
        )
        self.run_command(
            ["sudo", "systemctl", "enable", "postgresql"], "Failed to enable PostgreSQL"
        )
        ...
        for command in commands:
            ...
            self.run_command(
                ["sudo", "-u", "postgres", "psql", "-c", formatted_command],
                f"Failed to execute: {formatted_command}",
            )
        ...
        self.run_command(
            ...
            "Failed to modify pg_hba.conf for postgres user",
        )
        self.run_command(
            [
                "sudo",
            ...
            "Failed to modify pg_hba.conf for all users",
        )
        self.run_command(
            [
                "sudo",
            ...
            "Failed to modify postgresql.conf",
        )
        self.run_command(
            [
                "sudo",
    ...
    def check_and_update_python_deps(self):
        ...
        for package, install_args in required_packages.items():
            if not self.run_command(
                ["pip", "show", package], f"{package} is not installed"
            ):
                bt.logging.info(f"Installing {package}...")
                self.run_command(
                    ["pip", "install"] + install_args.split(),
    ...
    def clear_pycache(self):
        ...
        try:
            ...
            bt.logging.warning(
                "Failed to clear pycache using extended regex. Trying alternative method."
            )
            try:
