    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                bt.logging.info(
                    f"PostgreSQL not ready (attempt {attempt + 1}/{max_retries}). Retrying in {retry_delay} seconds..."
                )
...
def create_database_if_not_exists():
    try:
        ...
        cursor.execute(
            f"SELECT 1 FROM pg_catalog.pg_database WHERE datname = '{os.getenv('DB_NAME')}'"
        )
...
def setup_postgres(sqlite_db_path):
    ...
    if not wait_for_postgres():
        bt.logging.error(
            "Failed to connect to PostgreSQL. Please set up PostgreSQL manually."
        )
        return
