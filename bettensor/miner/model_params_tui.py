...
class ModelParamsTUI:
    ...
    def load_params(self):
        ...
        with self.db_connect() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT * FROM model_params WHERE id = %s", (self.miner_id,))
    ...
    def validate_input(self, key, value):
        try:
            if key in ['wager_distribution_steepness', 'fuzzy_match_percentage', 'top_n_games']:
                return int(value)
    ...
    def create_table(self, width):
        table = Table(title=f"Model Parameters for Miner ID: {self.miner_id}", 
                      box=box.ROUNDED, title_style=LIGHT_GOLD, width=width)
    ...
    def create_explanation(self, width):
        ...
        return Panel(explanation, title="Parameter Explanation", width=width, border_style=LIGHT_GREEN)

    def create_legend(self, width):
        legend = Table(title="Key Bindings", box=box.ROUNDED, title_style=LIGHT_GOLD, width=width)
    ...
    def get_formatted_text(self):
        ...
        available_table_height = height - legend_height - explanation_height - 6  # Subtract some padding
