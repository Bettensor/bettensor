...
class MinerDataMixin:
    ...
    def insert_predictions(self, processed_uids, predictions):
        ...
        """
        current_time = datetime.utcnow().replace(tzinfo=datetime.timezone.utc).isoformat()
        ...
        for uid, prediction_dict in predictions.items():
            for predictionID, res in prediction_dict.items():
                ...
                if self.db_manager.fetchone("SELECT COUNT(*) FROM predictions WHERE prediction_id = ?", (prediction_id,))[0] > 0:
                    bt.logging.debug(
                ...
                try:
                    ...
                    current_total_wager = self.db_manager.fetchone(
    ...
    def update_recent_games(self):
        ...
        for game in recent_games:
            ...
            start_time = datetime.fromisoformat(start_time).replace(tzinfo=datetime.timezone.utc)
    ...
    def get_recent_games(self):
        ...
        two_days_ago = (datetime.utcnow().replace(tzinfo=datetime.timezone.utc) - datetime.timedelta(hours=48)).isoformat()
...
def process_game_result(self, sport, game_response, external_id, team_a, team_b):
        ...
        bt.logging.trace(f"Game {external_id} result: {team_a} {home_score} - {away_score} {team_b}")
...
def get_sport_from_db(self, external_id):
    result = self.db_manager.fetchone("SELECT sport FROM game_data WHERE external_id = ?", (external_id,))
...
def fetch_game_data(self, current_timestamp: str) -> Dict[str, TeamGame]:
    ...
    rows = self.db_manager.fetchall(query, (current_timestamp, fifteen_days_ago, current_timestamp))
