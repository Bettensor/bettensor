...
class ScoringSystem:
    def __init__(
        ...
        self.window_sortino_scores = t.zeros(num_miners, max_days, len(self.tier_configs))
    ...
    def _update_raw_scores(self, predictions: List[t.Tensor], closing_line_odds: t.Tensor, results: t.Tensor):
        """
        ...
        for i, miner_predictions in enumerate(predictions):
            if miner_predictions.size(0) > 0:
                ...
                relevant_closing_odds = closing_line_odds[game_indices, predicted_outcomes.long() + 1]
                ...
                returns = t.where(prediction_results == predicted_outcomes, 
        ...
        self.logger.info(f"Entropy scores - min: {entropy_scores.min().item():.8f}, "
    ...
    def _update_window_scores(self):
        ...
        for tier, config in enumerate(self.tier_configs):
            ...
            self._set_tensor_for_day(self.window_clv_scores, self.current_day, clv_window.mean(dim=1), tier)
            self._set_tensor_for_day(self.window_roi_scores, self.current_day, roi_window.mean(dim=1), tier)
            
            # Calculate Sortino ratio
            target_return = 0  # You can adjust this if needed
            excess_returns = roi_window - target_return
            downside_returns = t.where(excess_returns < 0, excess_returns, t.zeros_like(excess_returns))
            ...
            sortino = t.clamp(sortino, min=-10, max=10)  # Cap Sortino ratio between -10 and 10
            self._set_tensor_for_day(self.window_sortino_scores, self.current_day, sortino, tier)

            self.logger.info(f"Tier {tier} Sortino ratio - min: {sortino.min().item():.4f}, "
    ...
    def _update_composite_scores(self):
        ...
        for tier in range(len(self.tier_configs)):
            clv = self._get_tensor_for_day(self.window_clv_scores, self.current_day, tier)
            roi = self._get_tensor_for_day(self.window_roi_scores, self.current_day, tier)
            sortino = self._get_tensor_for_day(self.window_sortino_scores, self.current_day, tier)
            ...
            composite_score = (composite_score - composite_score.mean()) / (composite_score.std() + 1e-8)
            
            self._set_tensor_for_day(self.composite_scores, self.current_day, composite_score, tier)
    ...
    def calculate_composite_scores(self):
        ...
        """
        composite_scores = t.zeros((self.num_miners, self.max_days, len(self.tier_configs)))
        ...
        for tier, config in enumerate(self.tier_configs):
            ...
            sortino = self._get_window_scores(self.window_sortino_scores[:, :, tier], start_day, window)
            entropy = self._get_window_scores(self.entropy_scores, start_day, window)
            
            if clv.numel() > 0 and roi.numel() > 0 and sortino.numel() > 0 and entropy.numel() > 0:
                composite_score = (
                ...
                self._set_tensor_for_day(composite_scores, self.current_day, composite_score, tier)
    ...
    def manage_tiers(self):
        ...
        try:
            ...
            assert new_tiers.max() < self.num_tiers, f"Tier index {new_tiers.max()} exceeds configured tiers."
            ...
            assert initial_miner_count == final_miner_count, f"Miner count changed from {initial_miner_count} to {final_miner_count}"
    ...
    def _handle_demotions(self, tiers, composite_scores):
        ...
        for tier in range(len(self.tier_configs), 1, -1):
            ...
            for miner in demoted_miners:
                ...
                self.logger.info(f"Miner {miner.item()} demoted from tier {tier} due to insufficient wager: {wager[miner == tier_miners].item():.2f} < {config['min_wager']}")
    ...
    def _handle_promotions_and_fill_slots(self, tiers, composite_scores):
        ...
        self.logger.info(f"Promotions and fills made: {(initial_tiers != tiers).sum().item()}")
    ...
    def _promote_and_swap(self, tier, tiers, composite_scores, config, next_config):
        ...
        if next_tier_miners.numel() == next_config['capacity']:
            next_tier_scores = composite_scores[next_tier_miners, self.current_day, tier]
            for i, (miner, score) in enumerate(zip(top_half_miners, top_half_scores)):
                ...
                if score > next_tier_scores[i]:
                    ...
                    self.logger.info(f"Miner {miner.item()} promoted to tier {tier + 1}, replacing miner {next_tier_miners[i].item()}")
                else:
    ...
    def _fill_empty_slots(self, tier, tiers, composite_scores, config):
        ...
        for miner in promoted_miners:
            self.logger.info(f"Miner {miner.item()} promoted to tier {tier} to fill empty slot")
    ...
    def _get_cumulative_wager(self, miners, window):
        ...
        if start_day <= end_day:
            ...
            wager = self.amount_wagered[miners, start_day:].sum(dim=1) + self.amount_wagered[miners, :end_day+1].sum(dim=1)
    ...
    def _cascade_demotion(self, miner, current_tier, tiers, composite_scores):
        ...
        for lower_tier in range(current_tier - 1, 0, -1):
            ...
            if lower_tier_miners.numel() < config['capacity']:
                ...
                lower_tier_scores = composite_scores[lower_tier_miners, self.current_day, lower_tier - 1]
                if miner_score > lower_tier_scores.min():
                    ...
                    tiers[miner] = lower_tier
                    self._cascade_demotion(lowest_miner, lower_tier, tiers, composite_scores)
                    break
        else:
    ...
    def log_score_summary(self):
        ...
        for score_name, score_tensor in [
            ...
            self.logger.info(f"{score_name} Scores - min: {current_scores.min().item():.4f}, "
    ...
    def calculate_weights(self):
        ...
        try:
            ...
            current_tiers = self._get_tensor_for_day(self.tiers, self.current_day)
            tier_counts = t.bincount(current_tiers, minlength=len(self.tier_configs)+1)[1:]
            ...
            for i, tier in enumerate(active_tiers):
                ...
                tier_scores = self._get_tensor_for_day(self.composite_scores, self.current_day, tier-1)[tier_mask]
    ...
    def scoring_run(self, current_date):
        ...
        predictions, closing_line_odds, results = self.scoring_data.preprocess_for_scoring(date_str)
        ...
        self.logger.info(f"Current day: {self.current_day}, reference date: {self.reference_date}")
        ...
        if predictions:
            ...
            total_wager = self._get_tensor_for_day(self.amount_wagered, self.current_day).sum().item()
            ...
            self.logger.warning(f"No predictions for date {date_str}. Skipping score update.")
    ...
    def reset_date(self, new_date):
        ...
        self._set_tensor_for_day(self.tiers, self.current_day, t.ones(self.num_miners, dtype=t.int))
