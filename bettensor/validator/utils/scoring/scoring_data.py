...
class ScoringData:
    ...
    def get_closed_games_for_day(self, date: str):
        try:
            query = """
                SELECT * FROM game_data WHERE event_start_date = ? AND active = 1
            """
            return self.db_manager.fetch_all(query, (date,))
        except Exception as e:
    ...
    def fetch_closing_line_odds(self):
        try:
            ...
            """
            games = self.db_manager.fetch_all(query, (current_time - timedelta(minutes=15), current_time + timedelta(minutes=15)))
            ...
            """
            for game in games:
                self.db_manager.execute_query(insert_query, game)
        except Exception as e:
    ...
    def preprocess_for_scoring(self, date: str):
        ...
        """
        try:
            games = self._fetch_game_data(date)
            if not games:
            ...
            game_id_map = {game[0]: i for i, game in enumerate(games)}
            closing_line_odds = [[i] + [float(odds) for odds in game[1:4]] for i, game in enumerate(games)]
            results = [float(game[4]) for game in games]

            predictions_data = self._fetch_predictions(date)
            
            closing_line_odds = t.tensor(closing_line_odds, dtype=t.float32)
            results = t.tensor(results, dtype=t.float32)
    ...
    def _fetch_game_data(self, date):
        query = """
        SELECT external_id, team_a_odds, team_b_odds, tie_odds, outcome
        FROM game_data
        WHERE event_start_date = ? AND active = 1
        """
        return self.db_manager.fetch_all(query, (date,))
